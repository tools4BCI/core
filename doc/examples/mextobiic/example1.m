...
for i = 1:100
	...
	icmessage_setbidx(EMG.message, i);
	...
	fnEMG = icmessage_getbidx(Fusion.EMG.message);
	if(fnEEG == fnEMG)
		icmessage_setbidx(Fusion.F.message, fnEEG);
	else
		disp('Error!');
	end
	...
	title(['Probabilities after fusion - Frame ' ...
		num2str(icmessage_getbidx(Fusion.F.message))]);
